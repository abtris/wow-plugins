...
function TitanPanelRollButton_OnLoad(self)
	self.registry = {
		id = TITAN_ROLL_ID,
...
function TitanPanelRollButton_OnClick(self, button)
	local i;
...
function TitanPanelRollButton_OnEvent(self, event, ...)
	local msg = ...;
...
function TitanPanelRollButton_ProcessRoll(player, roll, min_roll, max_roll)
	...
	if (TitanGetVar(TITAN_ROLL_ID, "PopupTooltip")) and rollRegistered then
		TitanPanelButton_SetTooltip(TitanPanelRollButton, TITAN_ROLL_ID);
	end
       end
end

function TitanPanelRollButton_OnUpdate(self)
	local secNow;
	...
	if (TitanRollsCount) then
		if not (TitanGetVar(TITAN_ROLL_ID, "ShowWinner")) then
			...
			if (maxloc > 0) then
				...
				buttonText = TitanUtils_GetColoredText(TITANROLL_NA, HIGHLIGHT_FONT_COLOR);
			end
		end
	else
		buttonText = buttonText..TitanUtils_GetColoredText(TITANROLL_NA, HIGHLIGHT_FONT_COLOR);
	end
	if not (TitanGetVar(TITAN_ROLL_ID, "ShowWinner")) then
...
function TitanPanelRollButton_ChangeAnnounce()
	...
	_G[_G[StaticPopup_Visible("TITANROLL_CHANGEANNOUNCE")]:GetName().."EditBox"]:SetText(TitanGetVar(TITAN_ROLL_ID, "AnnouncePattern"))
...
function TitanPanelRollButton_SetListLength(self)
	local i;
...
function TitanPanelRollButton_SetTimeout(self)
	TitanSetVar(TITAN_ROLL_ID, "Timeout", TITANROLL_TIMEOUTS[self.value]);
	if (TitanRollsCount) then
