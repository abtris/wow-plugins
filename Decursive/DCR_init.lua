--[[
    ...
    Decursive (v 2.6.0.1) add-on for World of Warcraft UI
...
function D:VersionWarnings()
    ...
    if (("2.6.0.1"):lower()):find("beta") or ("2.6.0.1"):find("RC") or ("2.6.0.1"):find("Candidate") or alpha then
        ...
        if D.VersionTimeStamp ~= 0 then
            ...
            if time() > D.VersionTimeStamp + VersionLifeTime then
                ...
                if time() - self.db.global.LastExpirationAlert > 48 * 3600  then
                    StaticPopup_Show ("Decursive_Notice_Frame", "|cff00ff00Decursive version: 2.6.0.1|r\n\n" .. "|cFFFFAA66" .. L["DEV_VERSION_EXPIRED"] .. "|r");
        ...
        if self.db.global.NonRealease ~= "2.6.0.1" then
            self.db.global.NonRealease = "2.6.0.1";
...
T._LoadedFiles["DCR_init.lua"] = "2.6.0.1";
