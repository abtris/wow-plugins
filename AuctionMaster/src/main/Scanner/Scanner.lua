...
function vendor.Scanner:FullScan(modules, func, arg)
	if (self.scanTask) then
		vendor.Vendor:Error(L["There is already a running scan."])
	else
		self.nextScanId = self.nextScanId + 1
    	if (modules) then
    		self.scanTask = vendor.ScanTask:new(self.nextScanId, {getAll = true}, unpack(modules))
    	else
    		self.scanTask = vendor.ScanTask:new(self.nextScanId, {getAll = true}, self.sniperModule, self.gatherModule)
    	end
		vendor.TaskQueue:AddTask(self.scanTask, func, arg)
		return self.nextScanId
    end
...
function vendor.Scanner:SearchScan(info, silent, modules, func, arg)
	if (self.scanTask) then
		...
		self.nextScanId = self.nextScanId + 1
		local queryInfo = {name = info.name, minLevel = info.minLevel or "", maxLevel = info.maxLevel or "", 
			invTypeIndex = 0, classIndex = info.classIndex or 0, subclassIndex = info.subclassIndex or 0, 
			isUsable = info.usable or 0, qualityIndex = info.rarity or 0}
		if (not itemLink) then
			log:Debug("sniper: [%s]", self.sniperModule)
			if (modules) then
				self.scanTask = vendor.ScanTask:new(self.nextScanId, queryInfo, unpack(modules))
...
function vendor.Scanner:UnregisterAuctionListUpdate()
	if (self.registeredAuctionListUpdate) then
		self:UnregisterEvent("AUCTION_ITEM_LIST_UPDATE")
		self.registeredAuctionListUpdate = nil
	end
end

function vendor.Scanner:RegisterAuctionListUpdate()
	if (not self.registeredAuctionListUpdate) then
		self:RegisterEvent("AUCTION_ITEM_LIST_UPDATE")
		self.registeredAuctionListUpdate = true
	end
end
...
function vendor.Scanner:ContainerFrameItemButton_OnModifiedClick(frame, button, ...)
	...
	log:Debug("but [%s] is [%s] id [%s] type [%s]", but, vendor.AuctionHouse:IsAuctionHouseTabShown(self.scanFrame.id), self.scanFrame.id, vendor.AuctionHouse.type)
	if (((but == "RightButton") or (but == "LeftButton" and IsShiftKeyDown())) and 
		vendor.AuctionHouse:IsAuctionHouseTabShown(self.scanFrame:GetTabType())) then
		log:Debug("1")
		local itemLink = GetContainerItemLink(frame:GetParent():GetID(), frame:GetID())
		if (itemLink) then
			log:Debug("found itemLink")
			self.scanFrame:PickItem(itemLink)
		end
	else
