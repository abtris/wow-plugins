...
local function _CancelUndercut(self)
	...
	for row=1,self.itemModel:Size() do
		local index, _, itemLink, name, _, count, bid, buyout, lowerBuyout, saleStatus, outDated, minBid, bidAmount = self.itemModel:Get(row)
...
function vendor.OwnAuctions:CancelAuctions(auctions)
	...
	for i=1,#auctions do
		...
		for j=1,self.itemModel:Size() do
			if (not tmp[j]) then
    			local index, itemLinkKey, itemLink, name, _, count, bid, buyout, lowerBuyout, saleStatus, _, 
    					minBid, bidAmount = self.itemModel:Get(j)
