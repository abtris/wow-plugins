...
local function _SelectPricingModifier(self, value, select)
	...
	if (select) then
		local frame = self.settingsFrames[PRICE_MODEL_ID]
		frame.pricingModifier:SelectValue(value or 0)
	end
	...
	for i=1,self.selected do
		...
		if (info) then
			...
			if (info.general) then
    			...
    			frame.duration:SelectValue(info.general.duration or 3)
...
--				frame.autoPricingModel:SetChecked(info.autoPricingModel)
				frame.pricingModel:SelectValue(info.pricingModel.pricingModel or vendor.Seller.PRIZE_MODEL_UNDERCUT)
...
local function _Save(self)
	...
	if (self.settingsFrames[GENERAL_ID].enabled or self.settingsFrames[PRICE_MODEL_ID].enabled) then
		...
		if (self.settingsFrames[GENERAL_ID].enabled) then
    		...
    		info.general.duration = frame.duration:GetSelectedValue()
...
--			info.autoPricingModel = frame.autoPricingModel:GetChecked()
			info.pricingModel.pricingModel = frame.pricingModel:GetSelectedValue()
			...
			local pricingModifier = frame.pricingModifier:GetSelectedValue()
...
local function _CreateSettingsGroup(self, id, title, parent, width, height)
	...
	-- revert button
	local revert = vendor.GuiTools.CreateButton(parent, "UIPanelButtonTemplate", 60, 16, L["Revert"], nil)
	revert.frame = frame
	revert:SetPoint("LEFT", f, "RIGHT", 10, 0)
	revert:SetScript("OnClick", _OnRevert)
	-- edit button
	local edit = vendor.GuiTools.CreateButton(parent, "UIPanelButtonTemplate", 60, 16, L["Edit"], nil)
	edit.frame = frame
...
--	maxUndercutF:SetPoint("LEFT", maxUndercut, "RIGHT", 0, 0)
	...
	-- pricing model
	pricingModel = vendor.DropDownButton:new(nil, nonAutoFrame, 85, L["Price calculation"], L["Selects the mode for calculating the sell prices.\nThe default mode Fixed price just select the last sell price."]);
	pricingModel:SetPoint("TOPLEFT", -10, -8)
	pricingModel:SetId(MODEL_DROP)
    local prizeModels = {}
	table.insert(prizeModels, {value = vendor.Seller.PRIZE_MODEL_FIX, text = L["Fixed price"]})
	table.insert(prizeModels, {value = vendor.Seller.PRIZE_MODEL_CURRENT, text = L["Current price"]})
	table.insert(prizeModels, {value = vendor.Seller.PRIZE_MODEL_UNDERCUT, text = L["Undercut"]})
	table.insert(prizeModels, {value = vendor.Seller.PRIZE_MODEL_LOWER, text = L["Lower market threshold"]})
	table.insert(prizeModels, {value = vendor.Seller.PRIZE_MODEL_MULTIPLIER, text = L["Selling price"]})
	table.insert(prizeModels, {value = vendor.Seller.PRIZE_MODEL_MARKET, text = L["Market price"]})
	...
	rememberPricingModel:SetPoint("LEFT", pricingModel.button, "RIGHT", -10, 0)
	...
	-- pricing modifier
	pricingModifier = vendor.DropDownButton:new(nil, nonAutoFrame, 100, L["Pricing modifier"], L["Specifies a modification to be done on the calculated prices, before starting an auctions."]);
    ...
    table.insert(pricingModifiers, {value = vendor.BuyoutModifier.NONE, text = L["None"]})
	table.insert(pricingModifiers, {value = vendor.BuyoutModifier.SUBTRACT_MONEY, text = L["Sub. money"]})
	table.insert(pricingModifiers, {value = vendor.BuyoutModifier.SUBTRACT_PERCENT, text = L["Sub. percent"]})
	table.insert(pricingModifiers, {value = vendor.BuyoutModifier.ADD_MONEY, text = L["Add money"]})
	table.insert(pricingModifiers, {value = vendor.BuyoutModifier.ADD_PERCENT, text = L["Add percent"]})
	...
	money.obj = self
	money:SetPoint("LEFT", pricingModifier.button, "RIGHT", 0, 4)
	...
	percent:SetPoint("LEFT", pricingModifier.button, "RIGHT", 0, 2)
	percent.obj = self
...
local function _CreateGeneralFrame(self, parent)
	...
	rememberDuration:SetPoint("LEFT", duration.button, "RIGHT", -15, 3)
	f = frame:CreateFontString(nil, "BACKGROUND", "GameFontHighlight")
...
local function _SelectItem(self, itemLink)
	local handled
	if (itemLink) then
		local itemName, _, itemRarity, _, _, itemType, itemSubType, itemStackCount, _, itemTexture, itemSellPrice = GetItemInfo(itemLink)
		if (itemName) then
...
function vendor.ItemSettings:DropDownButtonSelected(button, value)
	log:Debug("DropDownButtonSelected id [%s]", button:GetId())
	if (MODIFIER_DROP == button:GetId()) then
		_SelectPricingModifier(self, value)
	elseif (MODEL_DROP == button:GetId()) then
		_SelectPricingModel(self, value)
	end
end