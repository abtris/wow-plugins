...
local function _ApplySettings(self)
	local otherDisabled = self.db.profile.compactAuctionInfo
	vendor.Vendor.options2.args.tooltipHook.args.detailedGrp.args.showAvgMinBid.disabled = otherDisabled
	vendor.Vendor.options2.args.tooltipHook.args.detailedGrp.args.showAvgBuyout.disabled = otherDisabled
	vendor.Vendor.options2.args.tooltipHook.args.detailedGrp.args.showAllTimeLabel.disabled = otherDisabled
	vendor.Vendor.options2.args.tooltipHook.args.detailedGrp.args.showCurrentLabel.disabled = otherDisabled
	vendor.Vendor.options2.args.tooltipHook.args.detailedGrp.args.showCurrentMinBid.disabled = otherDisabled
	vendor.Vendor.options2.args.tooltipHook.args.detailedGrp.args.showCurrentBuyout.disabled = otherDisabled
	vendor.Vendor.options2.args.tooltipHook.args.detailedGrp.args.showLowerMinBid.disabled = otherDisabled
	vendor.Vendor.options2.args.tooltipHook.args.detailedGrp.args.showLowerBuyout.disabled = otherDisabled
	vendor.Vendor.options2.args.tooltipHook.args.compactGrp.args.compactMarketPrice.disabled = not otherDisabled
	vendor.Vendor.options2.args.tooltipHook.args.compactGrp.args.compactColor.disabled = not otherDisabled
end

--[[
	Sets the given value in the profile.
--]]
local function _SetValue(info, val1, val2, val3)
	if (info.arg == "compactColor") then
		self.db.profile[info.arg] = {r = val1, g = val2, b = val3}
	else
		self.db.profile[info.arg] = val1
	end
	_ApplySettings(self)
end
...
local function _GetValue(info)
	if (info.arg == "compactColor") then
		local color = self.db.profile[info.arg] or {}
...
local function _CreateOptions()
	vendor.Vendor.options2.args.tooltipHook = {
		type = "group",
		name = L["Tooltip"],
		desc = L["Tooltip"],
		args = {
			...
			compactGrp = {
				type = "group",
				inline = true,
				name = L["Compact auction information"],
				order = 3.5,
				args = {			
        			compactAuctionInfo = {
        				type = "toggle",
        				name = L["Activated"],
        				desc = L["Will display the number of current auctions and the corresponding lower buyout, if any. Otherwise it will display the number of historical values and the current market price."],
        				get = _GetValue,
        				set = _SetValue,
        				arg = "compactAuctionInfo",
        				order = 1,
        			},
        			compactColor = {
        				type = "color",
        				name = L["Only market price color"],
        				desc = L["The color for the market price label in the compact auction info if there are no current auctions."],
        				get = _GetValue,
        				set = _SetValue,
        				arg = "compactColor",
        				order = 3,
        			...
        			compactMarketPrice = {
        				type = "toggle",
        				name = L["Always market price"],
        				desc = L["Will display a compact market price information even if there are current auctions."],
        				get = _GetValue,
        				set = _SetValue,
        				arg = "compactMarketPrice",
        				order = 2,
        			},
        		}
			},
			detailedGrp = {
				type = "group",
				inline = true,
				name = L["Detailed auction information"],
				order = 4,
				args = {
        			showAvgMinBid = {
        				type = "toggle",
        				name = L["Average min bid"],
        				desc = L["Selects whether the average minimum bid prize should be shown in the GameTooltip."],
        				get = _GetValue,
        				set = _SetValue,
        				arg = "showAvgMinBid",
        				order = 4,
        			...
        			showAvgBuyout = {
        				type = "toggle",
        				name = L["Average buyout"],
        				desc = L["Selects whether the average buyout prize should be shown in the GameTooltip."],
        				get = _GetValue,
        				set = _SetValue,
        				arg = "showAvgBuyout",
        				order = 5,
        			...
        			showAllTimeLabel = {
        				type = "toggle",
        				name = L["All time auctions label"],
        				desc = L["Selects whether the label for all time auctions should be shown in the GameTooltip."],
        				get = _GetValue,
        				set = _SetValue,
        				arg = "showAllTimeLabel",
        				order = 5.5,
        			},			
        			showCurrentLabel = {
        				type = "toggle",
        				name = L["Current auctions label"],
        				desc = L["Selects whether the label for current auctions should be shown in the GameTooltip."],
        				get = _GetValue,
        				set = _SetValue,
        				arg = "showCurrentLabel",
        				order = 6,
        			},			
        			showCurrentMinBid = {
        				type = "toggle",
        				name = L["Current min bid"],
        				desc = L["Selects whether the current minimum bid prize should be shown in the GameTooltip."],
        				get = _GetValue,
        				set = _SetValue,
        				arg = "showCurrentMinBid",
        				order = 7,
        			},			
        			showCurrentBuyout = {
        				type = "toggle",
        				name = L["Current buyout"],
        				desc = L["Selects whether the current buyout prize should be shown in the GameTooltip."],
        				get = _GetValue,
        				set = _SetValue,
        				arg = "showCurrentBuyout",
        				order = 8,
        			},
        			showLowerMinBid = {
        				type = "toggle",
        				name = L["Current lower min bid"],
        				desc = L["Selects whether the current lower minimum bid prize should be shown in the GameTooltip."],
        				get = _GetValue,
        				set = _SetValue,
        				arg = "showLowerMinBid",
        				order = 9,
        			...
        			showLowerBuyout = {
        				type = "toggle",
        				name = L["Current lower buyout"],
        				desc = L["Selects whether the current lower buyout prize should be shown in the GameTooltip."],
        				get = _GetValue,
        				set = _SetValue,
        				arg = "showLowerBuyout",
        				order = 10,
	...
	vendor.Vendor.AceConfigRegistry:RegisterOptionsTable("AuctionMaster TooltipHook", vendor.Vendor.options2.args.tooltipHook)
	vendor.Vendor.AceConfigDialog:AddToBlizOptions("AuctionMaster TooltipHook", L["Tooltip"], "AuctionMaster")
end
...
function vendor.TooltipHook:OnInitialize()
	self.db = vendor.Vendor.db:RegisterNamespace("TooltipHook", {
		profile = {
			...
			showAllTimeLabel = true,
			showCurrentMinBid = false,
			showCurrentBuyout = true,
			showLowerMinBid = false,
			showLowerBuyout = true,
			compactAuctionInfo = false,
			compactMarketPrice = true,
