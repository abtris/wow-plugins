...
local RELEASE_NOTES_4 = HTML_PREFIX..L["4xReleases"]..HTML_SUFFIX
...
vendor.Help.RELEASES_4X = "4xReleases"
...
local function _OnGroupSelected(widget, event, value)
	local self = widget.userdata.obj
 	...
 	elseif (value == self.RELEASES_4X) then
 		self.html:SetText(RELEASE_NOTES_4)
 		self.frame:SetStatusText(L["4xReleasesStatus"])
...
local function _CreateFrame(self)
	...
	local tree = { 
    	...
    	{value = "4xReleases", text = L["4.x Releases"]},
