...
function vendor.InventoryHandle.prototype:Update()
	...
	for bag = 0, 4 do
		for slot = 1, GetContainerNumSlots(bag) do
			...
			if (itemLink) then
				local name, link, _, _, _, _, _, stackCount, _, texture = GetItemInfo(itemLink);
				if (name) then
					...
					if (info) then
						...
						self.inventory[key] = {name = name, link = link, stackCount = stackCount, texture = texture, count = count};
					end
				 end
			 end
		 end
...
function vendor.InventoryHandle.prototype:PickupItemStack(itemLink, stackCount)
	...
	bag, slot = self:FillSmallerStack(stacks, stackCount);
	if (not bag) then
