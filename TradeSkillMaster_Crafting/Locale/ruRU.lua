...
L["Accept"] = "Принять"
L["Add Crafted Items from this Group to Auctioning Groups"] = "Добавить крафты из этой группы в Auctioning группы" -- Needs review
...
L["Adds all items in this Crafting group to Auctioning group(s) as per the above settings."] = "Добавить все предметы в этой группе в Auctioning группы согласно настройкам выше."
...
L["Allows you to override the minimum profit settings for this profession."] = "Здесь можно изменить минимальное кол-во для добавления в очередь для этой предмета."
...
L["Crafts"] = "Крафты"
L["Create Auctioning Groups"] = "Создать группу"
...
L["Double Click Queue"] = "Добавить предметов при клике"
L["Druid"] = "Друид"
L["Elixir"] = "Эликсир"
...
L["Hunter"] = "Охотник"
L["If checked, any crafts which are already in an Auctioning group will be removed from their current group and a new group will be created for them. If you want to maintain the groups you already have setup that include items in this group, leave this unchecked."] = "Отметьте, чтобы все крафты, которые уже включены в группы, были удалены из них. Для них будут созданы новые группы. Если вы не хотите изменять настройки групп, в которые входят эти крафты, не отмечайте этот пункт."
L["If checked, the main TSM frame will close when you open the craft management window."] = "Отметьте, чтобы главное окошко TSM закрывалось при открытии окна управления крафтом."
...
L["Items will only be added to the queue if the number being added is greater than this number. This is useful if you don't want to bother with crafting singles for example."] = "Предметы будут добавлены в очередь только если их кол-во больше этого числа. Полезно, если вы не хотите крафтить единичные предметы."
...
L["Minimum Profit (in %)"] = "Миниальная прибыль (в%)"
...
L["Reset the current profile back to its default values, in case your configuration is broken, or you simply want to start over."] = "Сбросить настройки профиля (если настройки сбились или просто необходимо начать настройку заново)."
L["Restock Queue"] = "Создать очередь."
L["Restock Queue Overrides"] = "Изменение настройкисоздания очереди."
...
L["Select the crafts you would like to add to Auctioning and use the settings / buttons below to do so."] = "Выберите крафты "
L["Shaman"] = "Шаман"
L["Shield"] = "Щит"
...
L["This item will only be added to the queue if the number being added is greater than or equal to this number. This is useful if you don't want to bother with crafting singles for example."] = "Этот предмет будет добавлен в очередь только если их кол-во. больше данного числа. Полезная функция, если вы не хотите крафтить единичные предметы."
L["This setting determines where seen count data is retreived from. The seen count data can be retreived from either Auctioneer or TradeSkillMaster's AuctionDB module."] = "Эта функция задает, откуда будут браться данные о частоте появления этого товара на аукционе. Эти данные могут браться как из Auctioneer, так и из модуля AuctionDB."
L["This will set the scale of the craft management window. Everything inside the window will be scaled by this percentage."] = "Эта функция определяет масштаб окна управления крафтом. Все внутри этого окна будет масштабировано в соответствии этим процентом."
L["Total"] = "Всего"
...
L["When you double click on a craft in the top-left portion (queuing portion) of the craft management window, it will increment/decrement this many times."] = "Двойной щелчок на крафте наверху слева в окне управления крафтом уменьшит/увеличит его кол-во в очереди."
...
L["You can either add every craft to one group or make individual groups for each craft."] = "Для каждого крафта можно создать отдельную группу или добавить их все в одну группу."
